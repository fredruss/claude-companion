  ...
  _handleFile(file, stats, initialAdd) {
    ...
    const listener = async (path, newStats) => {
      ...
      if (!newStats || newStats.mtimeMs === 0) {
        try {
          const newStats2 = await promises$1.stat(file);
          if (this.fsw.closed)
            return;
...
class FSWatcher extends events.EventEmitter {
  ...
  async _emit(event, path, stats) {
    ...
    if (opts.alwaysStat && stats === void 0 && (event === EVENTS.ADD || event === EVENTS.ADD_DIR || event === EVENTS.CHANGE)) {
      ...
      let stats2;
      try {
        stats2 = await promises$1.stat(fullPath);
      } catch (err) {
      }
...
const STATUS_DIR = sysPath.join(os.homedir(), ".claude-companion");
const STATUS_FILE = sysPath.join(STATUS_DIR, "status.json");
let mainWindow = null;
async function ensureStatusDir() {
  if (!fs.existsSync(STATUS_DIR)) {
    await promises$1.mkdir(STATUS_DIR, { recursive: true });
  }
}
async function readStatus() {
  try {
    const content = await promises$1.readFile(STATUS_FILE, "utf-8");
...
function createWindow() {
  ...
  mainWindow = new electron.BrowserWindow({
    ...
    resizable: false,
    webPreferences: {
      preload: sysPath.join(__dirname, "../preload/index.js"),
  ...
  if (process.env.NODE_ENV === "development") {
    ...
    mainWindow.loadFile(sysPath.join(__dirname, "../renderer/index.html"));
  }
...
electron.ipcMain.handle("get-status", async () => {
  return await readStatus();
});
electron.ipcMain.on("start-drag", () => {
  if (mainWindow) {
    mainWindow.setIgnoreMouseEvents(false);
  }
});
